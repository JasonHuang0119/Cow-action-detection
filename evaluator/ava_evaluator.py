...
class AVA_Evaluator(object):
    def __init__(self, 
        ...
        self.mAP_history = []  # Add this line to store mAP history
        self.latest_results = {}  # 用于存储最新的评估结果
    ...
    def get_ava_mini_groundtruth(self, full_groundtruth): #這邊是grouth truth 找 1/4出來當驗證 我資料很小 所以直接拿全部groundtruth來檢驗
        """
        ...
        for i in range(3):
            for key in full_groundtruth[i].keys():
                if int(key.split(",")[1]) % 1 == 0:  # if int(key.split(",")[1]) % 4 == 0:  
    ...
    def calculate_mAP(self, epoch):
        ...
        self.latest_results = results  # 存储结果以供后续使用
    ...
    def plot_results(self):
        import matplotlib
        matplotlib.use('Agg')  # 设置为非 GUI 后端
        ...
        bars = plt.bar(categories, ap_values, color='#add8e6')
        ...
        plt.title('AP by Category and Overall mAP')
        plt.text(len(categories)-1, self.latest_results['PascalBoxes_Precision/mAP@0.5IOU'], f'mAP: {self.latest_results["PascalBoxes_Precision/mAP@0.5IOU"]:.2f}', color = 'red', ha='right', va='bottom')
        # 保存图表到文件
        plt.savefig('/home/jason/YOWOv2_cow/evaluator/mAP_plot.png')
        plt.close()  # 关闭图形，释放资源
