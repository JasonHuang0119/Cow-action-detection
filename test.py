...
def inference_ava(args, model, device, dataset, class_names=None, class_colors=None):
    ...
    for index in range(args.start_index, len(dataset)):
        ...
        # batch size = 1 # batch_bboxes 是整個影片的每一張全部 bboxes 
        # 這邊用 bboxes 抓 batch_bboxes 表示去抓當前畫面所有的 bboxes 也就是一次一張 bboxes (x1,y1,x2,y2,actions classes) (這邊 action class 有六個 我行為自己定義六種) (所以會是總共11個)
        bboxes = batch_bboxes[0]
        print(batch_bboxes[0],"batch_bboxes shape",batch_bboxes.shape) 
