...
def nms(bboxes, scores, nms_thresh):
    """"Pure Python NMS."""
    x1 = bboxes[:, 0]  #xmin #bboxes[:, 0]   我這邊修改為 np.round(bboxes[:, 0],3) 目的是因為我的 train.csv 的 bbox 數值是小數多位np.round(bboxes[:, 0],3)
    y1 = bboxes[:, 1]  #ymin  # np.round(bboxes[:, 1],3)   #bboxes[:, 1]   這邊是將它取到小數後三位 （原本 ava dataset 格式）
    x2 = bboxes[:, 2]  #xmax np.round(bboxes[:, 2],3)
    y2 = bboxes[:, 3]  #ymax #bboxes[:, 3] np.round(bboxes[:, 3],3)
    

    areas = np.maximum(1e-14, (x2 - x1)) * np.maximum(1e-14, (y2 - y1))
    #areas = (x2 - x1) * (y2 - y1)
