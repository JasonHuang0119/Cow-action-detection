...
這個專案基於 Yowov2 架構修改，模型為 YoloX + Yolov7 版本組合並結合 3D CNN 模型架構。應用於從牛舍中側拍的影像中提取牛隻影像並做目標追蹤牛隻，藉由牛隻隨時間變化的特徵透過模型處理進一步去分析各別牛隻行為統計，用於輔助農民觀測牛舍場域牛隻，並確定是否有異常行為、發情前的徵兆行為判斷，用以幫助農場管理。


## 功能
- 使用 YOWOv2 模型進行即時牛隻目標追蹤位置和行為檢測，幫助管理牛場確認牛是否為正常行為規範
