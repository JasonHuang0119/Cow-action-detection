...
class CSAM(nn.Module):
    ...
    def forward(self, x):
        ...
        B, C, H, W = x.size()
        # query / key / value
        query = x.view(B, C, -1)                    #[B,C,N] , N = H*W
        ...
        '''
        對 energy 進行最大池化操作，然後用這個最大值擴展成和 energy 相同的形狀，最後減去 energy。這樣的操作可以使得注意力機制更集中於局部信息,有助於強調特定通道之間的關係
        '''
...
class ChannelEncoder(nn.Module):
    ...
    def forward(self, x1, x2):
        """
            x: [B, C, H, W]
        """
        #print("Before concatenation - x1 size: {}, x2 size: {}".format(x1.size(), x2.size()))
